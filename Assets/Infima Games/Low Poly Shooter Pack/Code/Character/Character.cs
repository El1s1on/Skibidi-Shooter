...
namespace InfimaGames.LowPolyShooterPack
{
	...
	public sealed class Character : CharacterBehaviour
	{
		...
		private void AttackMelee()
		{
            ...
            if (Physics.Raycast(cameraWorld.transform.position, cameraWorld.transform.forward, out hit, 2f,17))
            {
				Debug.Log(hit.transform.name);
				if (hit.transform.GetComponent<Hitboxes>()) hit.transform.GetComponent<Hitboxes>().Hit(true);
            }
        }
		...
		public void OnTryFire(InputAction.CallbackContext context)
		{
			...
			if (!cursorLocked || Health.Instance.died)
				return;
